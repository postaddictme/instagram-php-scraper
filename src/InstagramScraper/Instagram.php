...
//        return md5(implode(':', [$this->getRhxGis(), $variables]));
    ...
    public function getMediaCommentsByCode($code, $count = 10, $maxId = null)
    {
        ...
        while ($hasPrevious && $index < $count) {
            ...
            if (static::HTTP_OK !== $response->code) {
                throw new InstagramException('Response code is ' . $response->code . '. Body: ' . json_encode($response->body) . ' Something went wrong. Please report issue.', $response->code);
            }
            $this->parseCookies($response->headers);
            $jsonResponse = $this->decodeRawBodyToJson($response->raw_body);
            ...
            if (
                ...
                return ['error' => static::getErrorBody($response->body), 'maxId'=>$maxId];
            }
    ...
    public function getPaginateAllFollowers($accountId, $pageSize = 20, $nextPage = '')
    {
        ...
        if ($response->code !== static::HTTP_OK) {
            throw new InstagramException(static::getErrorBody($response->body), $response->code);
        }
