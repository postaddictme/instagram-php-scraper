...
class Instagram
{
    ...
    const MAX_TAGGED_PER_REQUEST = 50;
...
//        return md5(implode(':', [$this->getRhxGis(), $variables]));
     ...
     * @throws InstagramNotFoundException
     */
    public function getAccountMediaTaggedByUsername($accountId, $count = 50, $maxId = '', $delayed = true)
    {
        $index = 0;
        $hasNextPage = true;
        $tagged_medias = [];
        ...
        while ($index < $count && $hasNextPage) {
            ...
            if ($count - $index > static::MAX_TAGGED_PER_REQUEST) {
                $numberOfTaggedToRetrieve = static::MAX_TAGGED_PER_REQUEST;
            } else {
                $numberOfTaggedToRetrieve = $count - $index;
            }

            $response = Request::get(Endpoints::getMediaTaggedLink($accountId, $numberOfTaggedToRetrieve, $maxId),
                $this->generateHeaders($this->userSession));
            ...
            $nodes = $jsonResponse['data']['user']['edge_user_to_photos_of_you']['edges'];
            $hasNextPage = $jsonResponse['data']['user']['edge_user_to_photos_of_you']['page_info']['has_next_page'];
            $numberOfTagged = $jsonResponse['data']['user']['edge_user_to_photos_of_you']['count'];
            $maxId = $jsonResponse['data']['user']['edge_user_to_photos_of_you']['page_info']['end_cursor'];


            if (empty($nodes) && $numberOfTagged === 0) {
                break;
            }

            foreach ($nodes as $mediaTag) {
                $tagged_medias[] = $mediaTag['node'];
            }

            if ($count > $numberOfTagged) {
                $count = $numberOfTagged;
            }
     ...
     * @throws InstagramNotFoundException
     */
    public function getAccountMediaTaggedCountByUsername($accountId, $delayed = true)
    {
        $hasNextPage = true;
        $index = 0; // total count
        $count = static::MAX_TAGGED_PER_REQUEST;
        $maxId = '';
        while ($hasNextPage) {
            $response = Request::get(Endpoints::getMediaTaggedLink($accountId, $count, $maxId),
                $this->generateHeaders($this->userSession));
            if ($response->code === static::HTTP_NOT_FOUND) {
                throw new InstagramNotFoundException('Account with given username does not exist.');
            }
